...
image_path = f"../datasets/VLKEB_images/mmkb_images/"
...
def run_api_llm(query, api_name, max_retry=3):
...
def run_llm_divide(query,local_modal):
...
def run_llm_choice(query,local_modal):
    if "api" in local_modal:
        # 如果没有本地模型，就使用API模型
        result = run_api_llm(query,local_modal)
    else:
...
def choose_entity_with_llm(subq, rag_entity, extract_entity, rag_wiki_id, extract_wiki_id, multimodal_graph, choose_base_prompt, local_modal, max_try=3):
    """
    ...
    for _ in range(max_try):
        ...
        choose_result = run_llm_choice(choose_prompt, local_modal)
...
def main():
    ...
    if model_name.startswith("api"):
        ...
        local_modal = ModelManager(model_name, half=False)

    question2subqs_path = f"question2subqs_{model_name}.json"
    ...
    # with open("../datasets/all_triple_embeddings.pkl", "rb") as f:
                ...
                for index_q,q in enumerate(last_triple['rewrite_question']):
                    ...
                    for index_sub, subq in enumerate(sub_questions):
                        ...
                        if index_sub ==0:
                            ...
                            input_query_embedding = Mretriever.encode_queries(input_query,have_image=True)
                            ...
                            print("Found entity from image: " + ", ".join(top_names))
                            entity = top_names[0]
                            ...
                            # print("condidate_triples", condidate_triples)
                                ...
                                condidate_embedding = Mretriever.encode_passages(condidate_triples)
                            ...
                            output = run_llm_answer(answer_prompt,local_modal)
                            print("Output from LLM: " + output)

                            output  = re.sub(r'^\s*Answer:\s*', 'Answer:', output , flags=re.IGNORECASE)
                            match = re.search(r'^\s*Answer:\s*', output)
                            print("match: " + str(match))
                            print("----------------")
                            
                            if match:
                                
                                rag_entity = match.group(1).replace("_", " ")
                            ...
                            print("Extracted key: " + str(extract_key))
                            ...
                            if extract_entity is not None:
                                if rag_entity is None :
                                    ...
                                    print("rag_wiki_id", rag_wiki_id)
                                    print("extract_wiki_id", extract_wiki_id)
                                    print("------------"*30)
                                    #如果相同直接用
                                    ...
                                    #如果不同就去选择
                                        entity = choose_entity_with_llm(subq, rag_entity, extract_entity, rag_wiki_id, extract_wiki_id, multimodal_graph, choose_base_prompt, local_modal, max_try=3)
                    ...
                    ans =  entity
                    print("Entity list: " + str(entity_list))
