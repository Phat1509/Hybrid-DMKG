...
class MultimodalRetriever:
    def __init__(self, model_name='nvidia/MM-Embed', device='cuda', half=True, use_blip=False):
        self.use_blip = use_blip
        self.device = device if torch.cuda.is_available() else 'cpu'
        if use_blip:
            self.processor = BlipProcessor.from_pretrained("Salesforce/blip-image-captioning-large")
            self.model = BlipModel.from_pretrained("Salesforce/blip-image-captioning-large").to(self.device)
            self.text_model = BlipTextModel.from_pretrained("Salesforce/blip-image-captioning-large").to(self.device)
    ...
    def encode_passages(self, passages, batch_size=10,have_image=True):
...
def main():
    ...
    Mretriever = MultimodalRetriever(use_blip=True)  # 或 use_blip=False
    ...
    for i in range(0, len(passages), batch_size):
        ...
        batch_embeddings = Mretriever.encode_passages(batch,have_image=True)  # 或 have_image=False
    ...
    with open('../datasets/all_triple_embeddings_blip2.pkl', 'wb') as f:
