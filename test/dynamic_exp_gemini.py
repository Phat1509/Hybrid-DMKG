...
def main():
    ...
    if model_name.startswith("api"):
        ...
        local_modal = ModelManager(model_name, half=False,port=5007)
    ...
    entity_cache = {}
    entity_cache_path = f"entity_cache_{divide_modal}.json"
    ...
    # 日志文件路径
    log_path = f"./results/log_{current_time}_{divide_modal}_{model_name}_image_{str(image_type)}.txt"
                            ...
                            if "[IMAGE]" in subq:
                                ...
                                if extract_entity is not None:
                                    if rag_entity is None :
                                        ...
                                        #如果不同就去选择
                                            entity = choose_entity_with_llm(subq, rag_entity, extract_entity, rag_wiki_id, extract_wiki_id, multimodal_graph, choose_base_prompt, local_modal, image_id, max_try=3)
