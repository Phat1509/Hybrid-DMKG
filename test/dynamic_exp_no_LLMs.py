...
def main():
    ...
    entity_cache = {}
    entity_cache_path = f"entity_cache_no_background_{divide_modal}_str{image_type}.json"
    ...
    with open("../datasets/all_triple_embeddings_no_background.pkl", "rb") as f:
    ...
    # 日志文件路径
    log_path = f"./results/log_{current_time}_{divide_modal}_not_LLMs_no_background_image_{str(image_type)}.txt"
    a=0
                ...
                for index_q,q in enumerate(last_triple['rewrite_question']):
                    ...
                    with open(log_path, "a", encoding="utf-8") as log_f:
                        ...
                        for index_sub, subq in enumerate(sub_questions):
                            log_f.write(f"Sub-questions: {sub_questions}\n")
                            print("subq: " + subq)  
                            #这里要去检索了
                            ...
                            if "[IMAGE]" in subq:
                                ...
                                if cache_key in entity_cache:
                                    ...
                                    topk_scores, topk_indices = Mretriever.compute_scores(input_query_embedding, image_embedding_list, topK=5)
